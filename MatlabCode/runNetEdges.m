...
%     img = imrotate(img, -90);
    img = imresize(img, 0.5);  % resize the image
    imgOrigin = img;
    ...
    edge = 1-edge;
    edge = edge > 0.95;
    combinedImg = edge.*imgOrigin;
    
    [xi2,yi2,P] = impixel(combinedImg);
    
    edge = bwselect(edge,xi2,yi2,4);
    
    edge = imresize(edge, 2); % scale up
    
    SE = strel('disk',8,8);
    edge = imerode(edge,SE);
    edge = imfill(edge,'holes');
    imwrite( uint8(edge*(2^8-1)),edgelist{its});
